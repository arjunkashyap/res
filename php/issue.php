...
echo "<div class=\"archive_title\">".$year."&nbsp;(Volume&nbsp;".intval($volume).")</div>";
?>
...
if($num_rows)
{
	for($i=1;$i<=$num_rows;$i++)
	{
		...
		echo "<div class=\"vol\">
					<a title=\"Issue No.&nbsp;".intval($issue)."\" href=\"toc.php?vol=$volume&issue=$issue\"><img src=\"cover/thumbs/cover_".$volume."_".$issue.".png\" alt=\"cover ".$volume." ".$issue."\"/></a>
					<p><a href=\"toc.php?vol=$volume&issue=$issue\">".$month_name{intval($month)}."&nbsp;(No.&nbsp;".intval($issue).")<br />pp.".$page_start."-".$page_end."</a></p>
				</div>";
	}
}
